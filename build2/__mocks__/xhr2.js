...
function isHeaderDisallowed(url, header) {
  var urlData = _mockUrls[url];
  return urlData != null && (urlData.disallowedHeaders || []).indexOf(header) >= 0;
}
...
function XMLHttpRequestMock() {
  ...
  this.setRequestHeader = jest.fn().mockImplementation(function (headerName, headerValue) {
    if (headerName.toLowerCase() === "range") {
      var matches = headerValue.match(/bytes=(\d+)-(\d+)/);
      _range = [Number(matches[1]), Number(matches[2])];
    }
  });
  this.getResponseHeader = jest.fn().mockImplementation(function (headerName) {
  ...
  this.getAllResponseHeaders = jest.fn().mockImplementation(function () {
    var headers = [];
