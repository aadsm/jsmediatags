...
jest.enableAutomock().dontMock("../jsmediatags.js").dontMock("../ByteArrayUtils.js");
...
describe("jsmediatags", function () {
  ...
  beforeEach(function () {
    ...
    jsmediatags.Config.removeTagReader(FLACTagReader); // Reset auto mock to its original state.

    NodeFileReader.canReadFile = jest.fn();
    NodeFileReader.prototype.init = jest.fn().mockImplementation(function (callbacks) {
      setTimeout(function () {
        callbacks.onSuccess();
      }, 1);
    });
    NodeFileReader.prototype.loadRange = jest.fn().mockImplementation(function (range, callbacks) {
      setTimeout(function () {
        callbacks.onSuccess();
      }, 1);
    });
  ...
  it("should read tags with the shortcut function", function () {
    ...
    ID3v2TagReader.prototype.read = jest.fn().mockImplementation(function (callbacks) {
      callbacks.onSuccess(mockTags);
    });
    return new Promise(function (resolve, reject) {
      jsmediatags.read("fakefile", {
        onSuccess: resolve,
        onError: reject
      });
      jest.runAllTimers();
  ...
  describe("tag readers", function () {
    ...
    it("should use the tag reader that is able to read the tags", function () {
      ...
      MockTagReader.getTagIdentifierByteRange = jest.fn().mockReturnValue([]);
      MockTagReader.canReadTagFormat = jest.fn().mockReturnValue(true);
      return new Promise(function (resolve, reject) {
        var reader = new jsmediatags.Reader();

        reader._getTagReader(new NodeFileReader(), {
          onSuccess: resolve,
          onError: reject
        });
    ...
    it("should load the super set range of all tag reader ranges", function () {
      ...
      return new Promise(function (resolve, reject) {
        ...
        reader._findTagReader(new NodeFileReader(), {
          onSuccess: resolve,
          onError: reject
        });
    ...
    it("should not load the entire file if two tag loaders require start and end ranges for tag identifier", function () {
      ...
      return new Promise(function (resolve, reject) {
        ...
        reader._findTagReader(fileReader, {
          onSuccess: resolve,
          onError: reject
        });
